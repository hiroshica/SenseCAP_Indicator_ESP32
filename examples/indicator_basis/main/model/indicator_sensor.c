...
struct sensor_history_data
{
    ...
    struct sensor_data_minmax data_week[WEEK_MAX];
};
...
static void __sensor_history_data_get( struct sensor_history_data  *p_history, struct view_data_sensor_history_data *p_data)
{
    ...
    memcpy(p_data->data_day, (uint8_t *)p_data_day, sizeof( struct sensor_data_average) * DAY_MAX);
...
static void __sensor_history_data_check(time_t now)
{
    ...
    xSemaphoreTake(__g_data_mutex, portMAX_DELAY);
    __sensor_history_data_day_check(__g_sensor_history_data.temp.data_day,   "Temp", now - SECOND_ADJUST);
    __sensor_history_data_week_check(__g_sensor_history_data.temp.data_week, "Temp",  now - SECOND_ADJUST * DAY_MAX);

    __sensor_history_data_day_check(__g_sensor_history_data.humidity.data_day,   "Humidity", now - SECOND_ADJUST);
    __sensor_history_data_week_check(__g_sensor_history_data.humidity.data_week, "Humidity",  now - SECOND_ADJUST * DAY_MAX);

    __sensor_history_data_day_check(__g_sensor_history_data.co2.data_day,   "CO2", now - SECOND_ADJUST);
    __sensor_history_data_week_check(__g_sensor_history_data.co2.data_week, "CO2",  now - SECOND_ADJUST * DAY_MAX);
...
static void __sensor_history_data_update_callback(void* arg)
{
    ...
    if( cur_day != last_day  &&  ((now - last_timestamp2) > (SECOND_ADJUST*DAY_MAX))) {
        ...
        last_timestamp2 = ((now) / (SECOND_ADJUST * DAY_MAX)) * (SECOND_ADJUST * DAY_MAX); //Sample at the day
    }
}
